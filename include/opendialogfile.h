...
namespace opendialog {
    ...
    std::wstring OpenFile(
        const wchar_t* title = L"File",                        // title of openDialogue 
        const _COMDLG_FILTERSPEC extensions = type::empty_ext, // extension list 
        const wchar_t* init_folder = nullptr)                 // initial folder to open
    {
        //Extension Array
...

    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            // extension 
            if (list_extension)
                pFileOpen->SetFileTypes(Number_Extensions, list_extension);
    ...
    std::wstring OpenFolder(const wchar_t* title, const std::wstring default_folder = L"")
    {
        //create pointer to wchar_t string for path name file;
        PWSTR pszFilePath;
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (default_folder != L"") {
                IShellItem* ishellitem = nullptr;
                hr = SHCreateItemFromParsingName(default_folder.c_str(), NULL, IID_PPV_ARGS(&ishellitem));
                if (SUCCEEDED(hr)) {
                    pFileOpen->SetFolder(ishellitem);
                    ishellitem->Release();
                }
    ...
    std::wstring BrowseFolder(std::string saved_path) {
        ...
        BROWSEINFO bi = { 0 };
#ifdef UNICODE
        bi.lpszTitle = reinterpret_cast<LPCWSTR>(L"Brows for folder...");
#else
        bi.lpszTitle = reinterpret_cast<LPCSTR>("Brows for folder...");
#endif // UNICODE
